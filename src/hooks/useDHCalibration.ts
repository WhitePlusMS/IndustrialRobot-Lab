...
import { buildCalibrationReport, type CalibrationReport } from '@/lib/dh-calibration';
...
function extractZeroFlangePose(arm: THREE.Group): Pose {
  ...
  // 米 → 毫米
  const positionMm: [number, number, number] = [pos.x * 1000, pos.y * 1000, pos.z * 1000];
  const rotation = quaternionToRotationMatrix([quat.x, quat.y, quat.z, quat.w]);
  const euler = rotationMatrixToEulerZYX(rotation);
