...
export default function WaypointPanel({ currentJoints, onGotoWaypoint }: WaypointPanelProps) {
  ...
  const { waypoints, saveWaypoint, deleteWaypoint, setOrigin, clearOrigin } = useWaypoints();

  const handleSave = () => {
    if (!name.trim()) return;
    saveWaypoint(name.trim(), currentJoints);
    setName('');
  };
          ...
          {waypoints && waypoints.length > 0 ? (
            waypoints.map((wp) => (
              ...
              >
                <button
                  type="button"
                  onClick={() => onGotoWaypoint([...wp.joints])}
                ...
                >
                  ...
                  <span className="block text-[10px] text-slate-500 font-mono mt-0.5 truncate">
                    J: {wp.joints[0].toFixed(1)},{wp.joints[1].toFixed(1)},{wp.joints[2].toFixed(1)},
                    {wp.joints[3].toFixed(1)},{wp.joints[4].toFixed(1)},{wp.joints[5].toFixed(1)}
                  </span>
                </button>
                {!wp.isOrigin && (
                  <button
                    ...
                    className="ml-1 p-1 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-sm transition-colors focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
                  >
