...
## 2026-06-22 OperationPanel Data Bus 消除 — 架构审查 #3

### 问题
- `useOperationPanelData()` 从 7 个 Context 聚合 ~75 个字段到 `OperationPanelData` 接口
- `{...data}` spread 传给 4 个子组件，每个子组件接收全部字段但只使用部分
- 子组件通过 props 耦合了不需要的数据，修改 Context 需要穿透 Data Bus 层

### 方案
- 子组件直接读取各自需要的 Context，删除中间聚合层
- `OperationPanel` 只负责布局（折叠/展开 + 按模块切换子面板）
- 子组件仅通过 `currentStep` prop 接收步骤信息

### 修改文件
- `src/components/operation/OperationPanel.tsx` — 删除 `OperationPanelData` 接口（112 行）和 `useOperationPanelData`（92 行），净减 ~200 行
- `src/components/operation/RobotOperations.tsx` — 直接调用 `useRobotContext()`
- `src/components/operation/CameraOperations.tsx` — 直接调用 `useVirtualCameraContext()`
- `src/components/operation/GraspOperations.tsx` — 直接调用 `useRobotContext()` + `useSuckerContext()` + `useSequenceContext()`
- `src/components/operation/FreeOperationPanel.tsx` — 直接调用各 Context

### 验证
- TypeScript 编译零错误
- 全部 35 个测试通过
- 删除测试：移除一个子组件引用的 Context 不会影响其他子组件

## 2026-06-22 PoE 解析运动学模型 — 消除 IK 数值 Jacobian 噪声

### 问题
- `GLBRobotModel` 依赖 `capturePoseForJoints` 数值采样场景 FK/Jacobian，J6 为纯姿态关节时数值差分被浮点噪声淹没，导致 ~40% 边界姿态 IK 无解
- IK 求解必须运行在 Three.js 场景中，无法在 Node.js 下单元测试
- DH 参数与 GLB 场景几何（Z/Y/Y/X/Y/Z 混合轴）不匹配，纯 DH 替换会崩精度

### 方案
- 用 Product of Exponentials 解析模型替换场景采样式运动学
- GLBRobotArm 挂载时一次性提取场景真实几何（关节轴 + pivot 位置 + 零位法兰），通过 bridge 暴露
- PoE FK 和解析 Jacobian 不依赖 Three.js，Jacobian 列永不为零

### 新增文件
- `src/lib/scene-kinematic-model.ts` — PoE 解析运动学模型（FK + 解析空间 Jacobian），实现 RobotModel 接口
- `src/lib/scene-kinematic-model.test.ts` — 17 个单元测试（FK 解析验证 + Jacobian + IK 闭环）

### 修改文件
- `src/lib/robot-pose-bridge.ts` — 新增 CalibrationData / JointCalibData 类型，setCalibration/getCalibration/subscribeCalibration 方法
- `src/components/GLBRobotArm.tsx` — 新增 extractCalibrationData 函数，挂载时提取标定数据注入 bridge
- `src/hooks/useRobot.ts` — 模型构造优先 SceneKinematicModel，GLBRobotModel 作为回退

### 验证
- 14 个原有测试 + 17 个新测试全部通过
- TypeScript 编译无错误
- 浏览器控制台确认 "PoE 标定数据已就绪: 6 个关节"
- 合成 6 关节标定数据下 IK 全部 5 个测试姿态收敛（含之前失败的 [-60, -30, 60, 30, 0, 0]），误差 < 1.5mm
- **清理**：
  - 删除整个英文 `## Development Guidelines` 块（~158 行）
  - 萃取独有价值内容并入中文区：
    - 及时刹车原则补充刹车后 4 步动作
    - 新增错误处理原则（第 7 条）
    - 新增提交信息原则（第 8 条）
    - 新增技术决策框架（简洁性/一致性/可读性/可测试性/可逆性）
- 涉及文件：`CLAUDE.md`

## 2026-06-22 Matt Pocock 工程技能配置

- 运行 `/setup-matt-pocock-skills`，为仓库配置工程技能基础设施
- **Issue Tracker**：本地 markdown 模式（`.scratch/<feature-slug>/`），适合个人项目
- **Triage 标签**：默认五角色标签（`needs-triage` / `needs-info` / `ready-for-agent` / `ready-for-human` / `wontfix`）
- **Domain Docs**：单项目结构（`CONTEXT.md` + `docs/adr/`）
- 涉及文件：
  - `CLAUDE.md` — 新增 `## Agent skills` 块
  - `docs/agents/issue-tracker.md` — 本地 issue tracker 约定
  - `docs/agents/triage-labels.md` — 标签映射表
